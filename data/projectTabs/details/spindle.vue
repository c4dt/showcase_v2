<template>
  ...
  <p>
    ...
    about
    <a class="epfl-a" href="https://en.wikipedia.org/wiki/Supervised_learning"
  ...
  <p>
    Supervised machine learning tries to match inputs to outputs, by finding
    links between both. First, the program is given a list of known links, with
    that, it creates what is called a "model" of the data, which can be used to
    ...
    pen based on some of its features. You first collect the products you can
    find locally and put them in the table below.
  </p>
  ...
  <p>
    It is already a bit cumbersome to determine theses relations, and that's
    only for 4 products. Imagine having hundreds of producers, thousands of
    products and many other features.
    <br />
    That's where using
    <a class="epfl-a" href="https://en.wikipedia.org/wiki/Unsupervised_learning"
    ...
    transformed in a single relation, so someone using it for prediction has to
    be aware that it might return wrong results. It also only tries to reproduce
    what happened and can't predict the future. For example, if a producer wants
    to make cheap black pens, the model will misbehave, stating that the pen
    will be red as all the previously seen cheap pens are red.
  </p>
  ...
  <p>
    ...
    log into your computer, you connect to a website or even when you receive a
    phone call.
    <br />
  ...
  <p>
    For example, if you want to send 7 to someone, you'll first encode it as 1
    and send it to the receiver, who will decode it back to 7. This way, we are
    able to send a message, so that only the person having the secret can decode
    it.
  </p>

  <p>
    Homomorphic encryption is a special kind of encryption where when you apply
    a mathematical operation between two encoded message, it will be the same as
    ...
    exercise left to the reader, you can try to add two clear messages (0 + 1 =
    1) and see if their encrypted counterpart are decoded correctly (4 + 5 = 9).
  </p>
